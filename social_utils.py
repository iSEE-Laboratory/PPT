...
# data_dir = os.path.dirname("/home/data1/xiaotong/Data/socialVAE_data/sdd")
H = {}
H_file = os.path.join("/home/data1/xiaotong/Data/socialVAE_data/sdd", "H_SDD.txt")
# print(1111, H_file)
...
def collect_data(set_name, dataset_type = 'image', batch_size=512, time_thresh=48, dist_tresh=100, scene=None, verbose=True, root_path="/home/data1/xiaotong/"):
	...
	rel_path = '/Data/socialVAE_data/sdd/{0}/'.format(set_name)
	...
	for file in glob.glob(root_path + rel_path + part_file):
		scene_name = file[len(root_path+rel_path):-6] + '_' + file[-5]
		...
		while len(list(data_by_id.keys()))>0:
			...
			# print(len(all_data_dict[curr_keys[0]]))
...
def generate_pooled_data(b_size, t_tresh, d_tresh, train=True, scene=None, verbose=True, root_path='./'):
	if train:
		full_train, full_masks_train = collect_data("train", batch_size=b_size, time_thresh=t_tresh, dist_tresh=d_tresh, scene=scene, verbose=verbose, root_path=root_path)
	...
	if not train:
		full_test, full_masks_test = collect_data("test", batch_size=b_size, time_thresh=t_tresh, dist_tresh=d_tresh, scene=scene, verbose=verbose, root_path=root_path)
...
generate_pooled_data(512, 0, 100, train=True, verbose=True, root_path="/home/data1/xiaotong/")
