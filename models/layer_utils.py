...
class NmpNet(nn.Module):
    ...
    def __init__(
        ...
        self.nmp_mlp_start = make_mlp([h_dim * 2 + embedding_dim, 128, h_dim], activation=None, batch_norm=batch_norm,
                                      dropout=0.5)
        self.nmp_mlps = self.make_nmp_mlp()
        self.nmp_mlp_end = make_mlp([h_dim, 128, bottleneck_dim], activation=activation, batch_norm=batch_norm,
                                    dropout=0.5)
            ...
            if self.nmp_layers <= 1:
                ...
                for nmp_l, nmp_mlp in enumerate(self.nmp_mlps):
                    if nmp_l % 2 == 0:
                        ...
                        edge_feat = nmp_mlp(self.node2edge(node_feat, rel_rec,
                                                           rel_send))  # [num_ped, h_dim] -> [num_edge, 2*h_dim] -> [num_edge, h_dim]
