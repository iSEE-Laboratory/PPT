...
class Dataloader(torch.utils.data.Dataset):
    ...
    def __init__(self, 
        dataset_name: str, dataset: str, ob_horizon: int, pred_horizon: int,
        ...
        device: Optional[torch.device]=None, is_train: bool=False,
        ...
        if dataset_name == "sdd":
            files = ["/home/data1/xiaotong/Data/socialVAE_data/sdd/train"] if is_train else ["/home/data1/xiaotong/Data/socialVAE_data/sdd/test"]
        else:
            fileName = "/home/data1/xiaotong/Data/socialVAE_data/"+dataset
            files = [fileName + "/train" if is_train else fileName + "/test"]
    ...
    def collate_fn(self, batch):
        ...
        for item in batch:
            ...
            # print(neighbor.shape, future.shape, hist.shape)
            ...
            if self.rotate and self.flip:
                hist_fliplr = fliplr(hist).copy()
                hist_fliplr = np.reshape(hist_fliplr, hist_shape)
                X.append(hist_fliplr)
                fut_fliplr = fliplr(future).copy()
                Y.append(fut_fliplr)
                # print(fut_fliplr.shape)
                neighbor_fliplr = fliplr(neighbor).copy()
                neighbor_fliplr = np.reshape(neighbor_fliplr, neighbor_shape)
                # print(neighbor_fliplr.shape)
                NEIGHBOR.append(neighbor_fliplr)
                ks = [1, 2, 3]
                for k in ks:
                    hist_rot = rot(hist, k).copy()
                    hist_fliplr = fliplr(hist_rot).copy()
                    hist_rot = np.reshape(hist_rot, hist_shape)
                    X.append(hist_rot)
                    hist_fliplr = np.reshape(hist_fliplr, hist_shape)
                    X.append(hist_fliplr)
                    fut_rot = rot(future, k).copy()
                    Y.append(fut_rot)
                    fut_fliplr = fliplr(fut_rot).copy()
                    Y.append(fut_fliplr)
                    neighbor_rot = rot(neighbor, k).copy()
                    neighbor_fliplr = fliplr(neighbor_rot).copy()
                    neighbor_rot = np.reshape(neighbor_rot, neighbor_shape)
                    NEIGHBOR.append(neighbor_rot)
                    neighbor_fliplr = np.reshape(neighbor_fliplr, neighbor_shape)
                    NEIGHBOR.append(neighbor_fliplr)
